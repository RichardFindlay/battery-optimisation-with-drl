...
class Battery(gym.Env):
	...
	def __init__(self, env_settings):
		...
		self.idx_ref = 1
		self.bug = False
		self.true_prices = pd.read_csv('./Data/N2EX_UK_DA_Auction_Hourly_Prices_2016_test.csv').iloc[:,-1]
	...
	def step(self, state, action, step):
		...
		if self.new_ep == True:
			self.ep_start_kWh_remain = current_soc * self.cr
		...
		# ts_reward =  np.squeeze(ts_price_kW * (action_kWh_clipped / (self.action_space[-1]* self.pr))) - (self.alpha_d * (abs(action_kWh_clipped) / (self.action_space[-1]* self.pr)))
	...
	def reset(self):
		...
		self.ep_end_kWh_remain = self.soc * self.cr
