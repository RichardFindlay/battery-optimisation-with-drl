...
# 	""" Policy Model """
# 	def __init__(self, state_size, action_size, seed, fc1_units=16, fc2_units=16, fc3_units=16, fc4_units=16):
...
# 		self.fc_value = nn.Linear(fc3_units, fc4_units)
# 		self.fc_adv = nn.Linear(fc3_units, fc4_units)
...
# 		value_fc = F.relu(self.fc_value(x))
# 		adv_fc = F.relu(self.fc_adv(x))
...
class DQN_Agent_double_duel():
	...
	def learn(self, experiences, gamma, tau):
		...
		# with torch.no_grad():
		# 	qsa_targets = self.qnet_target(next_states).detach().max(1)[0].unsqueeze(1)
